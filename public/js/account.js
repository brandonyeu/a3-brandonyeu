...
async function tryFetchSession() {
    ...
    for (const ep of endpoints) {
      try {
        ...
        if (!res.ok && res.status !== 401 && res.status !== 404) {
          // treat as success only when status is OK (200)
        }
        // try to parse JSON (if endpoint returns JSON)
        try {
          const data = await res.json();
          // If it looks like a session object, return it
          if (data && (data.loggedIn === true || data.loggedIn === false || data.user || data.username)) {
            return { ok: true, data };
          }
        } catch (e) {
          // not JSON or empty — continue trying other endpoints
        }
      } catch (err) {
  ...
  function showLoggedIn(username) {
    ...
    btn.addEventListener("click", async () => {
      ...
      if (ok) {
        // redirect to login or back to form (choose whichever you prefer)
  ...
  // small helper to guard against injection when injecting username into DOM
  ...
  window.addEventListener("load", async () => {
    ...
    if (!result.ok) {
      // If we couldn't determine session, show logged out UI
      showLoggedOut();
      return;
    }
    const s = result.data;
  
    // Normalize session shape: support a few common formats
